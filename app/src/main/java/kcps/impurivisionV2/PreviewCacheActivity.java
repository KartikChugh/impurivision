...
public class PreviewCacheActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_preview_cache);

        id = Images.imgId[getIntent().getIntExtra("num", 17)];
        progressText = findViewById(R.id.progressText);
        imageView = findViewById(R.id.img);
        setProgressText("Cropping Images", 1000);
        Bitmap bitmapInit = Bitmap.createBitmap(1920, 1080, Bitmap.Config.ARGB_8888);
        bitmapInit = Bitmap.createBitmap(bitmapInit, 420, 0, 1080, 1080);
        Bitmap bitmapPost = Bitmap.createBitmap(1920, 1080, Bitmap.Config.ARGB_8888);
    ...
    private void setProgressText(final String text, int delay) {
        ...
        }), delay + ((int)(Math.random() * 300 - 150)));
    }

    static class DifferenceTask extends AsyncTask<Bitmap, Void, int[]> {

        private WeakReference<PreviewCacheActivity> activityReference;
        private int width, height;

        DifferenceTask(PreviewCacheActivity context) {
            activityReference = new WeakReference<>(context);
        }

        @Override
        protected void onPostExecute(int[] colorsDiff) {
            final PreviewCacheActivity activity = activityReference.get();
            if (activity == null || activity.isFinishing()) return;
            activity.setProgressText("Running CNN", 2000);
            activity.setProgressText("Identifying Patterns", 3300);
            activity.setProgressText("Extracting Cache", 3800);
            activity.setProgressText("Aggregating Results", 4200);
            Handler h = new Handler();
            h.postDelayed((new Runnable() {
                @Override
                public void run() {
                    ...
                    if (id.equals("Water")) {
                        activityClass = SafeResultsActivity.class;
                    } else {
                        activityClass = UnsafeResultsActivity.class;
                    }
